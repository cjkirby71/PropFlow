    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto space-y-5" data-testid="properties-page">
      ...
      <div className="flex gap-3 flex-wrap" data-testid="properties-filters">
        <Select value={filterPropType} onValueChange={v => setFilterPropType(v === 'all' ? '' : v)}>
          <SelectTrigger className="w-[160px] h-9 text-sm bg-white"><SelectValue placeholder="Property Type" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={filterListType} onValueChange={v => setFilterListType(v === 'all' ? '' : v)}>
          <SelectTrigger className="w-[160px] h-9 text-sm bg-white"><SelectValue placeholder="Listing Type" /></SelectTrigger>
          <SelectContent>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-12 text-center" data-testid="properties-empty-state">
          <Building2 className="w-10 h-10 text-slate-300 mx-auto mb-3" />
          <p className="text-slate-500 text-sm mb-3">No properties listed yet.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" data-testid="properties-grid">
          {properties.map(p => (
            <div key={p.id} className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow" data-testid={`property-card-${p.id}`}>
              <div className="h-36 bg-slate-100 relative overflow-hidden">
                ...
                <div className="absolute top-2 right-2 flex gap-1">
                  ...
                  <Badge className={`text-xs ${p.listing_type === 'sale' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>{p.listing_type}</Badge>
                </div>
              </div>
              <div className="p-4">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3" /> {p.address}</p>
                  </div>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="p-1 hover:bg-slate-100 rounded" data-testid={`property-menu-${p.id}`}><MoreHorizontal className="w-4 h-4 text-slate-400" /></button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => setEditProp(p)}><Edit className="w-4 h-4 mr-2" /> Edit</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleDelete(p.id)} className="text-red-600"><Trash2 className="w-4 h-4 mr-2" /> Delete</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
                ...
                <Badge variant="outline" className={`mt-2 text-xs ${p.status === 'active' ? 'border-green-300 text-green-700' : 'border-slate-300 text-slate-500'}`}>{p.status || 'active'}</Badge>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input value={form.name} onChange={e => setForm({...form, name: e.target.value})} required className="bg-white border-slate-300" data-testid="property-form-name" />
      </div>
      <div>
        <Label className="text-sm font-medium text-slate-700 mb-1.5 block">Address *</Label>
        <Input value={form.address} onChange={e => setForm({...form, address: e.target.value})} required className="bg-white border-slate-300" data-testid="property-form-address" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <Label className="text-sm font-medium text-slate-700 mb-1.5 block">Listing Type</Label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input type="number" value={form.price} onChange={e => setForm({...form, price: parseFloat(e.target.value) || 0})} className="bg-white border-slate-300" data-testid="property-form-price" />
        </div>
        <div>
          <Label className="text-sm font-medium text-slate-700 mb-1.5 block">Sqft</Label>
          <Input type="number" value={form.sqft} onChange={e => setForm({...form, sqft: parseFloat(e.target.value) || 0})} className="bg-white border-slate-300" />
        </div>
      </div>
      <div className="grid grid-cols-3 gap-3">
        <div>
          <Label className="text-sm font-medium text-slate-700 mb-1.5 block">Beds</Label>
          <Input type="number" value={form.bedrooms} onChange={e => setForm({...form, bedrooms: parseInt(e.target.value) || 0})} className="bg-white border-slate-300" />
        </div>
        <div>
          <Label className="text-sm font-medium text-slate-700 mb-1.5 block">Baths</Label>
          <Input type="number" value={form.bathrooms} onChange={e => setForm({...form, bathrooms: parseInt(e.target.value) || 0})} className="bg-white border-slate-300" />
        </div>
        <div>
      ...
      <div>
        ...
        <Input value={form.image_url} onChange={e => setForm({...form, image_url: e.target.value})} placeholder="https://..." className="bg-white border-slate-300" />
      </div>
      <div>
